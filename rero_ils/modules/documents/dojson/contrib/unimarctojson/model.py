...
from rero_ils.modules.documents.dojson.contrib.marc21tojson.utils import get_mef_link
...
def unimarc_type_and_issuance(self, key, value):
    ...
    self["issuance"] = {}
    main_type = _ISSUANCE_MAIN_TYPE_PER_BIB_LEVEL.get(unimarc.bib_level, "rdami:1001")
    ...
    if not title:
        ...
        for tag in [
            ...
            for field in unimarc.get_alt_graphic_fields(tag=tag):
                title_data = {}
                part_list = TitlePartList(part_number_code="h", part_name_code="i")
                ...
                for blob_key, blob_value in items:
                    if blob_key == "a":
                        value_data = unimarc.build_value_with_alternate_graphic(
            ...
            if not unimarc.get_fields("214"):
                ...
                for blob_key, blob_value in items:
                    if blob_key in ("a", "c"):
                        ...
                        if index == 1:
                            old_type = "bf:Publication"
                            publication = {"type": publication_type, "statement": []}
...
def unimarc_identifier_isbn(self, key, value):
    ...
    identifiers = self.get("identifiedBy", [])
    if value.get("a"):
        isbn = {"type": "bf:Isbn", "value": value.get("a").replace("-", "")}
...
def unimarc_identifier_isbn_tag011(self, key, value):
    ...
    identifiers = self.get("identifiedBy", [])
    if value.get("a"):
        issn = {"type": "bf:Issn", "value": value.get("a")}
        if value.get("b"):
    ...
    if value.get("z"):
        for data in utils.force_list(value.get("z")):
            issn = {"type": "bf:Issn", "value": data, "status": "invalid"}
            identifiers.append(issn)

    if value.get("y"):
        for data in utils.force_list(value.get("y")):
            issn = {"type": "bf:Issn", "value": data, "status": "cancelled"}
            identifiers.append(issn)
    ...
    if value.get("g"):
        for data in utils.force_list(value.get("g")):
            issnl = {"type": "bf:IssnL", "value": data, "status": "cancelled"}
            identifiers.append(issnl)
...
def unimarc_electronicLocator_from_field_856(self, key, value):
    ...
    return {"url": value.get("u"), "type": "resource"} if value.get("u") else None
