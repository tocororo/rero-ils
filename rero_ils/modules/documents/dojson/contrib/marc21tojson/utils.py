...
_REPRODUCTION_SUBFIELDS_PER_TAG = {"533": "abcdemn", "534": "cep"}
...
_INTENDED_AUDIENCE_REGEXP = {
    ...
    "from the age of 14": re.compile(r"(Dès 14 ans|Ab 14 Jahre)", re.IGNORECASE),
    "from the age of 12": re.compile(r"(Dès 12 ans|Ab 12 Jahre)", re.IGNORECASE),
    "from the age of 10": re.compile(r"(Dès 10 ans|Ab 10 Jahre)", re.IGNORECASE),
    "from the age of 7": re.compile(r"(Dès 7 ans|Ab 7 Jahre)", re.IGNORECASE),
    "from the age of 5": re.compile(r"(Dès 5 ans|Ab 5 Jahre)", re.IGNORECASE),
    "from the age of 2": re.compile(r"(Dès 2 ans|Ab 2 Jahre)", re.IGNORECASE),
}
...
_SCALE_TYPE = {"a": "Linear scale", "b": "Angular scale", "z": "Other"}
...
def do_issuance(data, marc21):
    ...
    main_type = _ISSUANCE_MAIN_TYPE_PER_BIB_LEVEL.get(marc21.bib_level, "rdami:1001")
    ...
    if main_type == "rdami:1001":
        if sub_type not in ["article", "materialUnit", "privateFile", "privateSubfile"]:
            error = True
            sub_type = "materialUnit"
    elif main_type == "rdami:1002":
        if sub_type not in ["set", "partIndependentTitle", "partDependantTitle"]:
            error = True
            sub_type = "set"
    elif main_type == "rdami:1003":
        if sub_type not in ["serialInSerial", "monographicSeries", "periodical"]:
            error = True
            sub_type = "periodical"
...
def do_abbreviated_title(data, marc21, key, value):
    ...
    * partName = $i
    ...
    title = {}
    if value.get("a"):
        main_title = not_repetitive(marc21.bib_id, marc21.bib_id, key, value, "a")
        title_type = "bf:KeyTitle" if key[:3] == "222" else "bf:AbbreviatedTitle"
        title = {"type": title_type, "mainTitle": [{"value": main_title}]}
        if value.get("b"):
            subtitle = not_repetitive(marc21.bib_id, marc21.bib_id, key, value, "b")
...
def build_agent(marc21, key, value):
    ...
    if key[:3] in ["100", "700"]:
        ...
        if value.get("b"):
            numeration = not_repetitive(marc21.bib_id, marc21.bib_id, key, value, "b")
            agent_data["numeration"] = remove_trailing_punctuation(numeration)
        if value.get("c"):
            qualifier = not_repetitive(marc21.bib_id, marc21.bib_id, key, value, "c")
            agent_data["qualifier"] = remove_trailing_punctuation(qualifier)
        if value.get("d"):
            date = not_repetitive(marc21.bib_id, marc21.bib_id, key, value, "d")
            date = date.rstrip(",")
...
def do_contribution(data, marc21, key, value):
    ...
    if value.get("a"):
        ...
        if ref := get_mef_link(
            ...
            agent = {"$ref": ref, "_text": agent_data["authorized_access_point"]}
        else:
            agent = agent_data
        ...
        if value.get("4"):
            ...
            for role in utils.force_list(value.get("4")):
                ...
                if len(role) != 3:
                    error_print(
                        "WARNING CONTRIBUTION ROLE LENGTH:",
                        marc21.bib_id,
                        marc21.rero_id,
                        role,
                    )
                ...
                if role not in _CONTRIBUTION_ROLE:
                    error_print(
                        "WARNING CONTRIBUTION ROLE DEFINITION:",
                        marc21.bib_id,
                        marc21.rero_id,
                        role,
                    )
...
def do_provision_activity(data, marc21, key, value):
    ...
    def build_statement(field_value, subtags=("a", "b")):
        def build_agent_data(code, label, index, link):
            ...
            if not label:
                return None
            agent_data = {"type": type_per_code[code], "label": [{"value": label}]}
            with contextlib.suppress(Exception):
                alt_gr = marc21.alternate_graphic[tag][link]
                subfield = marc21.get_subfields(alt_gr["field"])[index]
                if subfield := remove_trailing_punctuation(subfield):
                    agent_data["label"].append(
                        {
                            "value": subfield,
                            "language": marc21.get_language_script(alt_gr["script"]),
                        }
                    )
        ...
        for blob_key, blob_value in items:
            if blob_key in subtags:
                agent_data = build_agent_data(blob_key, blob_value, index, link)
                if agent_data:
    ...
    subfield_3 = not_repetitive(marc21.bib_id, marc21.rero_id, key, value, "3")
    if subfield_3:
        notes = []
    ...
    if subfields_c := utils.force_list(value.get("c", [])):
        ...
        with contextlib.suppress(Exception):
            ...
            subfield = marc21.get_subfields(alt_gr["field"], code="c")
            date["label"].append(
                {
                    "value": subfield[0],
                    "language": marc21.get_language_script(alt_gr["script"]),
    ...
    if tag == "260":
        ...
        if subfields_g := utils.force_list(value.get("g", [])):
            ...
            date = {"label": [{"value": subfield_g}], "type": "Date"}
...
def do_identified_by_from_field_022(data, value):
    ...
    type_for = {"a": "bf:Issn", "l": "bf:IssnL", "m": "bf:IssnL", "y": "bf:Issn"}
    ...
    for subfield_code in ["a", "l", "m", "y"]:
        if subfields_data := value.get(subfield_code):
            ...
            for subfield_data in subfields_data:
                subfield_data = subfield_data.strip()
                identifier = {"type": type_for[subfield_code], "value": subfield_data}
...
def do_identified_by_from_field_024(data, marc21, key, value):
    ...
    subfield_2_regexp = {
        "doi": {"type": "bf:Doi"},
        "urn": {"type": "bf:Urn"},
        "nipo": {"type": "bf:Local", "source": "NIPO"},
        "danacode": {"type": "bf:Local", "source": "danacode"},
        "vd18": {"type": "bf:Local", "source": "vd18"},
        "gtin-14": {"type": "bf:Gtin14Number"},
    }

    type_for_ind1 = {
        "0": {"type": "bf:Isrc"},
        "1": {"type": "bf:Upc"},
        "2": {"pattern": r"^(M|9790|979-0)", "matching_type": "bf:Ismn"},
        "3": {"pattern": r"^97", "matching_type": "bf:Ean"},
        "8": {
    ...
    if subfield_a:
        if re.search(r"permalink\.snl\.ch", subfield_a, re.IGNORECASE):
            ...
            identifier.update({"value": subfield_a, "type": "uri", "source": "BNF"})
        elif subfield_2:
            ...
            if ind1 in ("0", "1", "2", "3", "8"):
                ...
                if "type" in type_for_ind1[ind1]:  # ind1 0,1
                    ...
                    if re.search(type_for_ind1[ind1]["pattern"], tmp):
                        identifier["type"] = type_for_ind1[ind1]["matching_type"]
                    else:
                        identifier["type"] = "bf:Identifier"
            else:  # ind1 not in (0, 1, 2, 3, 8)
                identifier.update({"value": subfield_a, "type": "bf:Identifier"})
...
def do_electronic_locator_from_field_856(data, marc21, key, value):
    ...
    if value.get("u"):
        ...
        indicator2 = key[4]
        content = utils.force_list(value.get("3"))[0] if value.get("3") else None
        public_note = []
   ...
   """
    ...
    if value.get("g"):
        work_access_point["miscellaneous_information"] = remove_trailing_punctuation(
            not_repetitive(bib_id, bib_id, key, value, "g"), ",."
        )
...
def _do_work_access_point_creator(marc21, key, value):
    ...
    # bf:Organisation
    if tag == "710":
        data = {"type": EntityType.ORGANISATION, "conference": False}
        if value.get("a"):
...
def do_work_access_point_240(marc21, key, value):
    ...
    for blob_key, blob_value in get_field_items(value):
        if blob_key in {"a"}:
            title = remove_trailing_punctuation(blob_value.replace("\u009c", ""))
...
def do_scale_and_cartographic(data, marc21, key, value):
    ...
    * créer un objet de "cartographicAttributes" pour chaque 255
    ...
    if value.get("a"):
        ...
        index = len(scales)
        scale = {"label": remove_trailing_punctuation(value.get("a"))}
        if fields_034 and len(fields_034) > index:
            ...
            if subfields_034_b:
                ...
                try:
                    scale["ratio_linear_horizontal"] = int(ratio_linear_horizontal)
                except Exception:
                    error_print(
            ...
            if subfields_034_c:
                ...
                try:
                    scale["ratio_linear_vertical"] = int(ratio_linear_vertical)
                except Exception:
                    error_print(
    ...
    def format_date_b(date):
        ...
        if test_min_max(year, 0, MAX_INT_YEAR):
            ...
            if test_min_max(month, 1, 12):
                ...
                day = date[7:9].zfill(2) if test_min_max(date[7:9], 1, 31) else "01"
                date_str = f"{date_str}-{day}"
                hour = date[9:11]
                if test_min_max(hour, 0, 23):
                    minute = date[11:13] if test_min_max(date[11:13], 0, 59) else "00"
                    second = date[13:15] if test_min_max(date[13:15], 0, 59) else "00"
