...
invenio reroils utils marc21tojson -t rt_000414 rt_000414.xml rt_000414.json rt_000414.xml_errors.xml -v
"""
import contextlib
import re
...
from .utils import SCHEMA_DOC, SCHEMA_ITEM, SCHEMA_LOFI, get_item, value_to_str
...
def marc21_to_subjects(self, key, value):
    ...
    # Correct wrong 655 $2 RERO-RAMEAU
    if subfield_2 in ['rero', 'RERO-RAMEAU']:
        ...
        if not subject.get('$ref'):
            # correct missing (RERO-RAMEAU) in 655 $0
            if tag_key == '655':
                if subfields_0:
                    identifier = {
                        'type': 'RERO-RAMEAU',
