...
class SyncAgent(object):
    ...
    def agent_are_different(self, agent1, agent2):
        ...
        def remove_fields(agent):
            ...
            for field in ['$schema', 'organisation', '_created', '_updated']:
                agent.pop(field, None)
            for source in agent['sources']:
                for field in ['$schema', 'md5']:
    ...
    def update_agents_in_document(
        ...
        # build the $ ref urls
        mef_url = current_app.config.get('RERO_ILS_MEF_AGENTS_URL')
        old_agent_url = f'{mef_url}/{source}/{old_agent_pid}'
        new_agent_url = f'{mef_url}/{source}/{new_agent_pid}'

        # get all agents from the document over all agent fields:
        # contribution and subjects
        agents = doc.get('subjects', []) + \
            [c['agent'] for c in doc.get('contribution', []) if c.get('agent')]
        ...
        if not agents_to_update:
            ...
            # the doc can not contains the old_agent_url
            # as it shared the same mef PID with an other agent source
            return False
        ...
        for agent in agents_to_update:
            ...
            # update the MEF pid if it exists
            has_mef_pid = agent.get('pid')
            if has_mef_pid:
                agent['pid'] = new_mef_pid
        # in any case we update the doc as the mef can be changed
        if not self.dry_run:
            if (old_agent_pid == new_agent_pid):
                # index only
                doc.reindex()
            else:
                # update in db and reindex
                ...
                if self.agent_are_different(dict(agent), mef):
                    ...
                    if not self.dry_run:
                        if (old_mef_pid == new_mef_pid):
                            Contribution.get_record_by_id(
                                agent.id).replace(new_mef_data, True, True)
                        else:
                    ...
                    for doc_pid in doc_pids:
                        doc_is_updated = self.update_agents_in_document(
                            doc_pid=doc_pid,
                            source=source,
                            old_agent_pid=old_agent_pid,
                            new_agent_pid=new_agent_pid,
                            new_mef_pid=new_mef_pid
                        )
                        if doc_is_updated:
